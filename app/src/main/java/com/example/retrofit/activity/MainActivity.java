...
public class MainActivity extends RxAppCompatActivity implements View.OnClickListener {
    ...
    private void uploadeDo() {
        String filePath = "/storage/emulated/0/Download/11.jpg";
        String fileMediaType = "image/image";
        RxAppCompatActivity activityImplementLifecycleProvider = this;

        upload(filePath, fileMediaType, activityImplementLifecycleProvider, listener, httpOnNextListener);
    }

    private static void upload(String filePath, String fileMediaType, RxAppCompatActivity activityImplementLifecycleProvider, UploadProgressListener listener, HttpOnNextListener httpOnNextListener) {
        ...
        MultipartBody.Part part = MultipartBody.Part.createFormData("file_name", file.getName(),
                new ProgressRequestBody(requestBody, listener));
        UploadApi uplaodApi = new UploadApi(httpOnNextListener, activityImplementLifecycleProvider);
