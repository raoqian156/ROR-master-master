...
public class DownHolder extends BaseViewHolder<DownInfo> implements View.OnClickListener {
    ...
    public void setData(DownInfo data) {
        ...
        itemData = data;
        progressBar.setMax((int) itemData.getCountLength());
        progressBar.setProgress((int) itemData.getReadLength());
        /*第一次恢复 */
        switch (itemData.getState()) {
            ...
            case ERROR:
                ...
                break;
            case FINISH:
                tvMsg.setText("下载完成 -> " + itemData.getSavePath());
                break;
        }
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_rx_down:
                if (itemData.getState() != DownState.FINISH) {
                    manager.startDown(itemData);
                    Log.e("DownHolder", "onClick.itemData -> " + itemData.getSavePath() + "|" + itemData.getId());
                }
                break;
            case R.id.btn_rx_pause:
                manager.pause(itemData);
                break;
        }
    }
